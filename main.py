...
def get_prime_implicants(term_arr):
    ...
    for term1 in term_arr:
        for term2 in term_arr:
            if term1 != term2:
                similar_indexes = [i for idx in range(len(term1)) if (term1[idx] == term2[idx])]
...
minterms = [int(i) for i in input("Please enter minterms: ").strip().split()]
var_count = int(input("Please enter number of variables: "))
default_count = len(bin(max(minterms))) - 2  # Default var count
if var_count < default_count:
    print("Insufficient variable count using " + str(default_count) + " instead")
    var_count = default_count
